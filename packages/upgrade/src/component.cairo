...
pub mod UpgradeableComponent {
    ...
    use sai_access::{AccessTrait, HasAccessComponent};
    use starknet::{ClassHash, SyscallResultTrait};
    use crate::{IUpgrade, IUpgradeAndCall};
    ...
    #[embeddable_as(UpgradeImpl)]
    impl Upgrade<
        TContractState,
        +HasComponent<TContractState>,
        +HasAccessComponent<TContractState>,
        +Drop<TContractState>,
    > of IUpgrade<ComponentState<TContractState>> {
        fn upgrade(ref self: ComponentState<TContractState>, new_class_hash: ClassHash) {
            let contract = self.get_contract();
            contract.assert_caller_is_owner();
            InternalTrait::upgrade(ref self, new_class_hash);
        }
    }

    #[embeddable_as(UpgradeAndCallImpl)]
    impl UpgradeAndCall<
        TContractState,
        +HasComponent<TContractState>,
        +HasAccessComponent<TContractState>,
        +Drop<TContractState>,
    > of IUpgradeAndCall<ComponentState<TContractState>> {
        fn upgrade_and_call(
            ...
            let contract = self.get_contract();
            contract.assert_caller_is_owner();
            InternalTrait::upgrade_and_call(ref self, new_class_hash, selector, calldata)
        }
    }
